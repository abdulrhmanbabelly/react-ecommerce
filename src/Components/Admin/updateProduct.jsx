...
import { FormGroup, FormLabel, MenuItem, Select, Slider, TextField, FormControl, Button, Box, Alert } from "@mui/material";
...
let UpdateProduct = (props) => {
    ...
    let { triggerNotification } = useNotification(`updated product ${title}`, 'info')
    ...
    let handleUpdate = async () => {
        ...
        updateProduct({
            ...
            if (!data.errors) triggerNotification(`updated product ${title}`, "success");
            else triggerNotification(`failed to update product ${title}`, "error");
